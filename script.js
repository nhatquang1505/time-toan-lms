...
const SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbx3uVQQwtdFIdWtPJWC7Zs0ue5GG5KbCdil4NKKWb0TI-97LzHM9WD-px772jP3cP1gLg/exec';
...
function openChangePasswordModal() {
  if (!currentUser || currentUser.type !== 'student') {
    showToast("Vui lòng đăng nhập tài khoản Học sinh để đổi mật khẩu!", false);
    return;
  }
...
function closeChangePasswordModal(e) {
  if (e && e.target && e.target.id !== 'change-password-modal' && !e.target.classList.contains('news-modal-close') && e.target.tagName !== 'BUTTON') {
    return;
  }
  const modal = document.getElementById('change-password-modal');
  if (modal) {
...
async function handleChangePassword(e) {
  ...
  if (newPass.length < 4) {
    showToast("Mật khẩu mới phải có ít nhất 4 ký tự!", false);
    return;
  }
  ...
  try {
    ...
    params.append('maHS', currentUser.maHS);
    ...
    if (btnSubmit) {
      ...
      btnSubmit.innerHTML = '💾 Lưu mật khẩu mới';
    }
  }
}
...
function showMainApp() {
  ...
  if (loaderHyphen) loaderHyphen.style.display = 'none';
  if (initialLoader) {
  ...
  if (mainContentHyphen) {
    mainContentHyphen.classList.remove('app-hidden');
    mainContentHyphen.style.display = '';
  }
  if (mainEl) {
    mainEl.classList.remove('app-hidden');
...
const showAppContent = showMainApp;
const hideLoaderAndShowContent = showMainApp;
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // SAFEGUARD TIMEOUT (3 Seconds Max Safeguard)
  // Guarantees UI opens within 3s even if server cold-start or network hangs
  setTimeout(() => {
    showMainApp();
  }, 3000);
  ...
  Promise.all([
    ...
    // Step 2: Everything ready -> show main app
    showMainApp();
  });
